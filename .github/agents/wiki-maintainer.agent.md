...
**Programmi di allenamento** (sorgenti sotto `raw/programs/**`): produci SEMPRE **due pagine wiki** per ogni programma ingerito — la source bibliografica in `wiki/sources/<slug>.md` (creata dal workflow standard) e la program detail in `wiki/programs/piano_<slug_ridotto>.md` (creata manualmente con `edit/editFiles` dopo la source). La program detail deve contenere: tabella massimali ingresso/target/uscita, struttura del microciclo, progressione settimanale per ogni fondamentale, sedute tecniche complementari, ausiliari ricorrenti, tabella di compliance se il raw contiene log di esecuzione, cross-link a concetti e coach. Registra la program detail nell'index sotto la sezione `Programs`. Vedi la regola dettagliata in `.github/instructions/wiki-conventions.instructions.md` § "Regola doppia-pagina per programmi". Dimenticare la program detail rende il programma solo un riferimento bibliografico, non queryable come struttura.
